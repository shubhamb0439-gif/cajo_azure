/*
  # Update Fulfill Delivery to Track Sales History

  ## Overview
  Updates the fulfill_delivery function to properly track the item_stock_sold field
  for finished products when they are sold.

  ## Changes
  - Increment item_stock_sold when reducing finished product stock
  - This provides accurate sales history for reporting and analytics
*/
...
BEGIN
  ...
  FOR v_sale_item IN
    ...
    WHERE di.delivery_id = p_delivery_id
  LOOP
    -- Reduce stock of the FINISHED PRODUCT by 1 and increment sales history
    UPDATE inventory_items
    SET 
      item_stock_current = item_stock_current - 1,
      item_stock_sold = COALESCE(item_stock_sold, 0) + 1,
      updated_by = p_user_id,
      updated_at = now()
