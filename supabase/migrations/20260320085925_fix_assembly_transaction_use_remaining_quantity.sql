/*
  # Fix Assembly Transaction Function - Use Correct Column Name

  ## Changes
  - Fix execute_assembly_transaction to use `remaining_quantity` instead of non-existent `quantity_used`
  - The purchase_items table tracks remaining stock in the `remaining_quantity` column
  - Update FIFO logic to properly deduct from remaining_quantity

  ## Technical Details
  - Changed all references from `quantity_used` to use `remaining_quantity`
  - FIFO logic: (quantity - remaining_quantity) gives the already-used amount
  - Available amount: remaining_quantity
  - After deduction: remaining_quantity = remaining_quantity - deduct_qty
*/
...
BEGIN
  ...
  LOOP
    ...
    FOR v_purchase_item IN
      SELECT pi.id, pi.quantity, pi.remaining_quantity, pi.unit_cost
